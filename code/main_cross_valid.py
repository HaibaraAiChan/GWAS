...
def load_data(folder, p_val):
	n_len = 0
	p_len = 0
	for fld in os.listdir(folder):
		if fld == "negative":
			for filenames in fld:
				with open(os.path.join(folder, filenames)) as myfile:
...
def train_mlp(folder,p_val, batch_size, lr, epoch, output):
	...
	vector, label = load_data(folder,p_val)
	y = np_utils.to_categorical(label, num_classes=2)

	# valid_voxel, valid_label = load_valid_data(voxel_output)
...
if __name__=="__main__":
	args = myargs()
	# args = argdet()
	train_mlp(args.folder, args.bs, args.lr, args.epoch, args.output)
